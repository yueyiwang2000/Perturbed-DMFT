...
In this file, we count B insertion as order 1.
"""
...
def iterative_perturbation(SigDMFT1,SigDMFT2,U,T,B,nfreq,order,countB=1,maxit=21):
    '''
    ...
    count B: 1: with B counterterm in Lagrangian, which is for no B field in Lattice Lagrangian;
             0: without B counterterm in L, which is for Lattice Lagrangian with B, used for susceptibility.
    '''
    period=5
    mu=U/2
    beta=1/T
    ...
    if rank ==0:
        ...
        # plt.plot(G11_taubf[:,0,0,0].real,label='G11_taubf real')
        # plt.plot(G11_taubf[:,0,0,0].imag,label='G11_taubf imag')
        # plt.legend()
        # plt.grid()
        # plt.show()
        ...
        delta_inf=np.abs(-mu+SigDMFT1[-1].real-B)# delta for accurate FFT
    ...
    #sig_tau=np.sum(Pq_tau*Gkq_tau,axis=(1,2,3))*(-1)*U**2/knum**3/beta  
        P11_tau=mpi_module.bubble_mpi(fft.precalcP_fft,knum,nfreq,11,G11_tau,G11_tau,0)
        P12_tau=mpi_module.bubble_mpi(fft.precalcP_fft,knum,nfreq,12,G12_tau,G12_tau,1)
        Sig2_11=mpi_module.bubble_mpi(fft.precalcsig_fft,knum,nfreq,11, G11_tau,P11_tau,beta,U,0)
    ...
    if order>=3:
        ...
        Q11_tau=mpi_module.bubble_mpi(fft.precalcQ_fft,knum,nfreq,11, G11_tau,G11_tau,0)
        # Q12_tau=P12_tau
        #FT
      ...
      #precalcsig has the factor. (-1)*U**2/knum**3. actually factor needed is U**3. need extra -U.
        ...
        Sig3_1_111=-U*mpi_module.bubble_mpi(fft.precalcsigp_fft,knum,nfreq,11,G11_tau,A_111_tau,beta,U,0 )
        Sig3_1_121=-U*mpi_module.bubble_mpi(fft.precalcsigp_fft,knum,nfreq,11,G11_tau,A_121_tau,beta,U,0 )
        Sig3_1_112=-U*mpi_module.bubble_mpi(fft.precalcsigp_fft,knum,nfreq,12,G12_tau,A_112_tau,beta,U,1 )

        Sig3_2_111=-U*mpi_module.bubble_mpi(fft.precalcsig_fft,knum,nfreq,11,G11_tau,B_111_tau,beta,U,0 )
        Sig3_2_121=-U*mpi_module.bubble_mpi(fft.precalcsig_fft,knum,nfreq,11,G11_tau,B_121_tau,beta,U,0 )
        Sig3_2_112=-U*mpi_module.bubble_mpi(fft.precalcsig_fft,knum,nfreq,12,G12_tau,B_112_tau,beta,U,1 )

        Sig3_ins_11,Sig3_ins_12=diagrams.insertion_Bequ1_order3(G11_iom,G12_iom,G11_tau,G12_tau,P11_tau,P12_tau,beta,B,knum,nfreq,U)

        Sig3_11=Sig3_1_111+Sig3_2_111+Sig3_1_121+Sig3_2_121+Sig3_ins_11*countB
        Sig3_12=Sig3_1_112+Sig3_2_112+Sig3_ins_12*countB
        ...
        # But, if our lagrangian is under B field, we don't pay the price, and we don't need these B insersions.
    ...
    while it < maxit and order > 0:
        ...
        if rank ==0:
            ...
            if order >=1:# first order. Be careful of the sign of B!
                new_Sigma11=(ori_Sigma11+B*countB+(nloc22-n0loc22)*U)[:,None,None,None]*np.ones((2*nfreq,knum,knum,knum))
                new_Sigma22=(ori_Sigma22-B*countB+(nloc11-n0loc11)*U)[:,None,None,None]*np.ones((2*nfreq,knum,knum,knum))
        ...
        it+=1
    if rank==0:
        filename='./data/{}_{}_{}_{}.dat'.format(B,U,T,order)
        if countB==0:
            filename='./data/{}_{}_{}_{}_countB0.dat'.format(B,U,T,order)
        f = open(filename, 'w')
...
def run_perturbation(U,T,B,nfreq,ordernum,countB):
    filename='../files_variational/{}_{}_{}/Sig.out.{}'.format(B,U,T,index)
    if (os.path.exists(filename)):
        ...
        Sig11,Sig22,Sig12=iterative_perturbation(sigA,sigB,U,T,B,nfreq,ordernum,countB)
    else:
...
if __name__ == "__main__":
    ...
    if (len(sys.argv)==5) or(len(sys.argv)==6):# use command line parameters to input the parameters:
    ...
    if (len(sys.argv)==6):
        countB=int(sys.argv[5])
        print('countB=',countB)
    run_perturbation(U,T,B,nfreq,ordernum,countB)