...
import maxent_routines as me# type: ignore
...
def Pade(om, Gm, x, gamma, Norder):
    '''
    om: matsubara freq grid.
    Gm: GF defined on imag axis.
    x: real axis grid.
    gamma: ~0.001 to 0.005  it's the 'idelta or iGamma' in the GF expression in textbooks.
    Norder: ~100. I guess this is how many matsubara freqs are used.
    something might be helpful:  arXiv:1705.10016v
    '''
    zn = om[:Norder]*1j
    gn = Gm[:Norder]
    Pt = me.padecof(gn, zn)
...
if __name__ == '__main__':
    ...
    beta = pi/om[0]
    tau = linspace(0,beta,params['Ntau']+1)# type: ignore

    for i in range(len(dat)/2):
        Gm=dat[2*i+1]+dat[2*i+2]*1j
        Gt = InverseFourier(Gm, om, tau, beta,  params['Nf'], params['statistics'])# type: ignore
        savetxt('Gtau.dat.'+str(i), vstack((tau,Gt)).transpose() )
        (Aw, omega) = MaximumEntropy(params, tau, Gt)# type: ignore
