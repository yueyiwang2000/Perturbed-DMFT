...
class FuncNDiagNew:
    ...
    def update(self,new_momentum,new_imagtime,newsublatindshort,i_coeff,l,j_coeff):
        ...
        for i in np.arange(self.nGf):
            ...
            self.kslice[i]=np.sum(self.depend[i,:self.Ndimk,None]*new_momentum,axis=0)
            self.tslice[i]=np.sum(self.depend[i,self.Ndimk:]*new_imagtime.T)
            # print('k{}=\n'.format(i),ki)
    ...
    def update_temp(self,iloop,new_momentum,new_imagtime,newsublatindshort,i_coeff,l,j_coeff):
        ...
        if iloop<self.Ndimk:# changing variable k 
            ...
            for i in np.nonzero(update)[0]:
                self.kslicetemp[i]=np.sum(self.depend[i,:self.Ndimk,None]*new_momentum,axis=0)# just reslice the k.
            ...
            update=self.depend[:,iloop]
            for i in np.nonzero(update)[0]:
                self.tslicetemp[i]=np.sum(self.depend[i,self.Ndimk:]*(new_imagtime.T))
