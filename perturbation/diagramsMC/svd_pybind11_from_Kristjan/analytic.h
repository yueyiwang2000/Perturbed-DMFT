...
private:
  ...
  bl::Array<double,1> to_integrate;
public:
  bl::Array<double,1> kx;
  Spline1D<double> epsx;
public:
  HartreeFock(double _kF_, double _cutoff_, double _beta_, double _lmbda_=0, /*double _dmu_=0,*/ int Nq=513) :
  ...
  double cmp()
  {
    bl::Array<double,1> epsn(epsx.size());
    bl::Array<double,1> Sigx(epsx.size());
    double dmu=0; // the shift of the chemical potential due to the finite-T
	...
	Sigx(i) = -sx * 2*kF/pi;
	epsn(i) = kx(i)*kx(i) - kF*kF + 2*kF/pi * (-sx + sx_kF);
      }
      double dn0 = Density(dmu, epsn);     // At finite-T the density might not be correct
      double dn = dn0;
      bool FoundSignChange  = false;
      double ddmu = -dn*20.;               // How big steps should we make to find a change of sign. Might need to be tuned to be applicable in general.
      int j;
      for (j=1; j<100; ++j){               // Looging for a sign-change
	dn = Density(dmu+ddmu*j, epsn);
	if (dn*dn0 < 0){
	  FoundSignChange =  true;
	  break;             // sign change occured, now we can find mu by searching for zero
	}
      }
      if (not FoundSignChange)
	std::cerr << "ERROR : Could not find sign change in Hartree Fock looking for the chemical potential dmu" << std::endl;

      double dmu2 = zeroin(dmu, dmu+ddmu*j, // Brent's method  using lambda function (new in C++11)
			   [this,epsn](double dx)->double{
			     return Density(dx,epsn);
			   },
			   1e-6);
      //clog << "a=" << dmu << " b=" << dmu+ddmu*j << " dmu=" << dmu2 << endl;
      dmu = dmu2;                           // This is now  the chemical potential
      for (int i=0; i<epsx.size(); i++){
	diff += fabs( (epsn(i)-dmu) - epsx[i]);
	epsx[i] = epsn(i)-dmu;              // Dispersion adjusted, and checking for change with iterations
      }
      epsx.splineIt(kx);
      //clog<<"diff="<<diff<<endl;
      if (diff<1e-6) break;                 // self-consistent, hence break
    }
//#define DEBUGX
#ifdef DEBUGX    
    ofstream out("hf_sigx.dat"); out.precision(12);
    out << "# " << dmu << endl;
    for (int i=0; i<kx.size(); ++i) out << kx(i) << " " << Sigx(i) << endl;
    lmbda=0;
    ofstream outw("hf_s0.dat"); outw.precision(12);
    for (int i=0; i<epsx.size(); i++){
      double sx = -2*kF/pi * Sx( kx(i), epsx);
      outw << kx(i) << " " << sx << endl;
    }
#endif    
    return dmu;
  ...
  double Density(double dmu, const bl::Array<double,1>& epsn)
  {
    //clog << "to_integrate.size=" << to_integrate.size() << " kx.size=" << kx.size() << " epsn.size="<<epsn.size()<<endl;
    for (int i=0; i<kx.size(); ++i)
      to_integrate(i) = kx(i)*kx(i) * ferm( (epsn(i)-dmu)*beta );
    double intg = simpson_nonuniform(kx, to_integrate);
    double N0 = 2*4*pi/ipower( 2*pi, 3 ) * intg;
    /*
    ofstream  debug("debug.dat"); debug.precision(15);
    debug <<"# " << intg << endl;
    for (int i=0; i<kx.size(); ++i)
      debug << kx(i) << " " << to_integrate(i) << endl;
    */
    double n0 = kF*kF*kF/(3*pi*pi); // n0=3/(4*pi*rs**3) and rs = pow( (9*pi/4), 1/3.)/kF
    //cout << "N0=" << N0 << " " << "n0=" << n0 << " intg=" << intg << endl;
    return N0-n0;
  }
