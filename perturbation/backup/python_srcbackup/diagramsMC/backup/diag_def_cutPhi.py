...
class FuncNDiagNew:
    ...
    def __init__(self,T,U,knum,taunum,nfreq,norder,ut,kbasis,sublatind_basis,perm,GFs,dependence,symfactor):
        ...
        # self.time_detectvar=0.
        # self.time_detectGF=0.
        self.time_ktau=0.
    ...
    def update_temp(self,new_momentum,new_imagtime,newsublatindshort,i_coeff,l,j_coeff):
        ...
        self.ktemp=copy.deepcopy(self.k)
        time1=time.time()
        ...
        time6=time.time()
        # self.time_deepcopy+=(time1-time0)
        # self.time_detectvar+=(time2-time1)
        # self.time_detectGF+=(time3-time2)
        # self.time_updateGF+=(time4-time3)
        # self.time_sublatind+=(time5-time4)
        # self.time_basis+=(time6-time5)
    ...
    def update_temp_fast(self,iloop,new_momentum,new_imagtime,newsublatindshort,i_coeff,l,j_coeff):
        '''
        ...
        if iloop<self.Ndimk:# changing variable k 
            ...
            # which GFs should be updated?
            update1=self.depend[:,iloop]
            update=np.zeros(self.nGf)
            for i in np.arange(self.nGf):# all Gfs
                if self.depend[i,iloop]!=0:# this GF depend on this updated variable,
                    update[i]=1# this Gf should be updated
            print(update,update1)
            for i in np.nonzero(update)[0]:
                k=np.sum(self.depend[i,:self.Ndimk,None]*new_momentum,axis=0)# just reslice the k.
                # tau=np.sum(self.depend[i,self.Ndimk:]*(new_imagtime.T))
                self.slicedG_temp[i]=Gshift(self.GF[self.Gind[i]],k,tau,self.ksym_array[i],self.knum,self.taunum,self.taulimit[i])
            time3=time.time()
            self.time_ktau+=(time3-time2)
            ...
            for i,ele in enumerate(newsublatind):# changing internal variable sublatind
                if ele!=self.SubLatInd[i]:
                    # print((iloop-self.Ndimk-self.Ndimtau)*2,newsublatind,self.SubLatInd)
                # print('old sublatind:',self.SubLatInd,'new sublatind:',newsublatind)
                # print('GF #{} was {}, now is {}'.format(i,self.SubLatInd[i]+self.SubLatInd[self.perm[i]],newsublatind[i]+newsublatind[self.perm[i]]))
