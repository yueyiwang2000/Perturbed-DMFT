...
'''
This is the version of svd monte carlo of July 5th, 2024.

This code already supports permutation representation input.
by giving the index of cutted propagator and labelling, this can do monte carlo for any permutation.
It is verified by 2 diagrams of order 3, see the definition in the diag_Def_new.pt with the same date.
...
class FuncNDiagNew:
    ...
    def update(self,new_momentum,new_imagtime,l,cut):
        ...
        return res*self.U**self.norder/self.knum**(3*(self.Ndimk-1))*self.beta/self.taunum**(self.Ndimtau-1)
