...
This is the old version, and diagrams are defined highly manually in diag_def.py.
'''
comm = MPI.COMM_WORLD
rank = comm.Get_rank()
...
def IntegrateByMetropolis_svd(func, qx, p,seed,lmax,ut,ifprint=1):
    ...
    fQ = func(momentum,imagtime)*ut[l,imagtime[0]], V0norm * myweight( momentum,imagtime ) # fQ=(f(X), V0*f_m(X))
    ...
    for itt in range(p.Nitt):   # long loop
        ...
        if (trialaccept): # trial step successful. We did not yet accept, just the trial step.
            ...
            fQ_new = func(tmomentum,timagtime)*ut[tl,timagtime[0]], V0norm * myweight(tmomentum,timagtime) # f_new
            # if timagtime[0]<=1:
...
def MC_test(U,T,nfreq,knum):
    ...
    # --------definition of fucntions: test different quantities.--------

    # GFs=(G11_tau,G12_tau,G22_tau)

    # fun=diag_def.FuncNDiag_G(knum, taunum,nfreq, 1, 1, Gloc11_tau, 0)# g
    ...
    # fun=diag_def.FuncNDiag_order2_new(U,knum, taunum,nfreq, 3, 1, G12_tau, G12_tau, G12_tau,1,1,1)#sigma2off
    # fun=diag_def.FuncNDiag_order2_new(U,knum, taunum,nfreq, 3,1, G11_tau, G22_tau, G22_tau,0,0,0)#sigma2
    # fun=diag_def.FuncNDiag_order2_new(U,knum, taunum,nfreq, 3, 1, Gloc11_tau, Gloc22_tau, Gloc22_tau,0,0,0)#sigma2loc

    fun=diag_def.FuncNDiag3_1(T,U,knum, taunum,nfreq, 4, 2, G11_tau, G11_tau, G22_tau, G22_tau, G22_tau,0,0,0,0,0)#sigma3_1_111
    # fun=diag_def.FuncNDiag3_2(T,U,knum, taunum,nfreq, 4, 2, G11_tau, G11_tau, G22_tau, G22_tau, G22_tau,0,0,0,0,0)#sigma3_2_111
    # fun=diag_def.FuncNDiag_new(T,U,knum, taunum,nfreq, 4, 2, G12_tau, G12_tau, G12_tau, G12_tau, G22_tau,1,1,1,1,0)#sigma3121
    # fun=diag_def.FuncNDiag_new(T,U,knum, taunum,nfreq, 4, 2, G12_tau, G22_tau, G12_tau, G11_tau, G12_tau,1,0,1,0,1)#sigma3122
                ...
                for kz in np.arange(10):
                    if k_displayed[kx,ky,kz]==0:# if this point is not checked
                        ...
                        for q in all_unique_sym_kpoints:
                            ...
                            quant=sigma3_1_111[:,q[0],q[1],q[2]].real
