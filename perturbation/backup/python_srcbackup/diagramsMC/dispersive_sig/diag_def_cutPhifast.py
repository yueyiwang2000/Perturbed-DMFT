...
class FuncNDiagNew:
    ...
    def update_temp(self,iloop,new_momentum,new_imagtime,newsublatindshort,i_coeff,l,j_coeff,globalflip=0):
        '''
       ...
       '''
        ...
        if iloop<self.Ndimk:# changing variable k 
            ...
            self.kbasisslice_temp[gind3_indices] *= self.ksign[gind3_indices]    
            # for i in np.arange(self.nGf):
            ...
            result=sumslices_fast(self.nGf,self.symfactor,self.slicedG,self.tbasisslice,self.kbasisslice_temp,self.indbasisslice)
            time5=time.time()
            ...
            #     if self.t[i]==0:
            #         self.tbasisslice_temp[i]*=self.taulimitfactor[i]
            result=sumslices_fast(self.nGf,self.symfactor,self.slicedG,self.tbasisslice_temp,self.kbasisslice,self.indbasisslice)
            time5=time.time()
            ...
            result=sumslices_fast(self.nGf,self.symfactor,self.slicedG,self.tbasisslice,self.kbasisslice,self.indbasisslice_temp)
            time5=time.time()
