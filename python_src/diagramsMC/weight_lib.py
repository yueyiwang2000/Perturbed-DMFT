...
def self_Add_to_K_histogram(dk_hist, momentum, imagtime,self_K_hist, self_tau_hist,self_Ndimk,self_Ndimtau):
...
def self_fm(momentum,imagtime, self_self_consistent, self_integral0,  self_gxk,self_gxtau):
    ...
    if not self_self_consistent:# in the very beginning, the trial.
        ...
        for itau in range(1,len(imagtime)):
...
class meassureWeight:
    ...
    """
    def __init__(self,Ndimk, Ndimtau,knum,taunum):
        # self.Nbin = Nbin
        self.Nloops = Ndimk+Ndimtau
        self.Ndimk=Ndimk
        self.Ndimtau=Ndimtau
        self.knum=knum
        self.taunum=taunum
        # \int f0(k) d^3k, where f0(k) is given above
        self.integral0 =  knum**(3*(self.Ndimk-1))*taunum**(self.Ndimtau-1)# integration of trial fm, which is f0
    ...
    def Add_to_K_histogram(self, dk_hist, momentum, imagtime):
        self_Add_to_K_histogram(dk_hist, momentum, imagtime, self.K_hist,self.tau_hist,self.Ndimk,self.Ndimtau)
    ...
    def Normalize_K_histogram(self):
        ...
        # dnrm = self.Ndimk/sum(self.K_hist)
        # self.K_hist *= dnrm
    ...
    def recompute(self):
        ...
        for itau in range(1,self.Ndimtau):
        ...
        for itau in range(1,self.Ndimtau):# normalization of gtau_i
