...
def output_memory_usage():
    ...
    memory_usage = process.memory_info().rss / (1024 * 1024)  # 获取内存使用量，转换为MB
...
def iterative_perturbation(SigDMFT1,SigDMFT2,U,T,nfreq,order,maxit=20):
    '''
    ...
    if rank ==0:
        ...
        #2st order double counted term
        # if order ==2:
        sigimp11,sigimp22=imp.pertimp_func(G11imp_iom,G22imp_iom,delta_inf,beta,U,knum,order)
    ...
    while it < maxit and diff > epsilon:
        ...
        if order==3:
            ...
            C1_111_tau=2*(-1)*U*fft.precalc_C(P11_iom,P11_iom)/beta
            C1_121_tau=2*(-1)*U*fft.precalc_C(P12_iom,P12_iom)/beta# hence this one only uses P12, and Q12=P12, so C1_121=C2_121!
            C1_112_tau=2*2*(-1)*U*fft.precalc_C(P11_iom,P12_iom)/beta# This one include 112 and 121 so there is a factor of 2
            C2_111_tau=2*(-1)*U*fft.precalc_C(Q11_iom,Q11_iom)/beta
            C2_112_tau=2*2*(-1)*U*fft.precalc_C(Q11_iom,P12_iom)/beta
        ...
        if rank ==0:
            ...
            new_Sigma11=ori_Sigma11+Sig2_11-sigimp11[:,None,None,None]
            new_Sigma22=ori_Sigma22+Sig2_22-sigimp22[:,None,None,None]
            new_Sigma12=Sig2_12
            if order==3:
            ...
            print(f'\tit={it},\tdiff={diff:.5f},\ttime={timei-time0:.2f}s,\tmemory usage={output_memory_usage():.2f} MB')
            ...
            # plt.plot(Sig3_11[nfreq:,0,0,0].real+Sig2_11[nfreq:,0,0,0].real,label='Sig2+3_11 {} real'.format(it))
            # plt.plot(Sig3_11[nfreq:,0,0,0].imag+Sig2_22[nfreq:,0,0,0].imag,label='Sig2+3_11 {} imag'.format(it))            
...
def Delta_pert_DMFT(SigA,SigB,U,T,knum,nfreq,order):
    ...
    iom= 1j*om
    sig_new_11,sig_new_22,sig_new_12=iterative_perturbation(SigA,SigB,U,T,nfreq,order)
    if rank==0:
...
if __name__ == "__main__":
    ...
    if (len(sys.argv)==1):# this is for test
        ...
        sigA=(+U/2+0.1)*np.ones(nfreq,dtype=complex)+1j*sigma[:,2]#sig+delta
        sigB=(+U/2-0.1)*np.ones(nfreq,dtype=complex)+1j*sigma[:,2]#sig-delta
